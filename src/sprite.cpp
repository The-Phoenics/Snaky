...
{
	obj.update_char_at_play_field(posx, posy, Player);
}
...
int Sprite::sprite_movement(board& obj, Fruit& f_obj)
{
	...
	if (m_keystate == 'W')
	{
		...
		{
			obj.update_char_at_play_field(posx, posy, ' ');

			// update score by 5 when Snake's eats the fruit
			if (obj.get_char_at_board(posx - 1, posy) == f_obj.get_fruit())
			{
				update_score();
				f_obj.set_fruitEaten(true);
			}

			obj.update_char_at_play_field(posx - 1, posy, Player);
	...
	else if (m_keystate == 'A')
	{
		if (obj.get_char_at_board(posx, posy - 1) != '#')
		{
			...
			// update score by 5 when Snake's eats the fruit
			if (obj.get_char_at_board(posx, posy - 1) == f_obj.get_fruit())
			{
				update_score();
				f_obj.set_fruitEaten(true);
			}

			obj.update_char_at_play_field(posx, posy - 1, Player);
...
int Sprite::get_score() const { return m_score; }
